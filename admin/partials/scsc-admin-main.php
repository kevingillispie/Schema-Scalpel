...
foreach ( $scsc_loc_tabs as $key => $name ) {
	...
	$nav_tabs[] = ( new HTML_Refactory( 'button' ) )
		->attr( 'class', $classes )
		->attr( 'id', "nav-$formatted_name-tab" )
		->attr( 'type', 'button' )
		->attr( 'data-toggle', 'tab' )
		->attr( 'data-target', "#nav-$formatted_name" )
		->attr( 'role', 'tab' )
		->attr( 'aria-controls', "nav-$formatted_name" )
		->attr( 'aria-selected', 'homepage' === $formatted_name ? 'true' : 'false' )
		->child( esc_html( $name ) )
		->render();
}
...
foreach ( $tab_panes as $pane ) {
	...
	$tab_content .= ( new HTML_Refactory( 'div' ) )
		->attr( 'class', array( 'tab-pane', 'fade' ) )
		->attr( 'id', "nav-$pane" )
		->attr( 'role', 'tabpanel' )
...
if ( ! empty( $_GET['set_tab'] ) && 'posts' === $_GET['set_tab'] ) {
	...
	$aside_content = ( new HTML_Refactory( 'aside' ) )
		...
		->child(
			( new HTML_Refactory( 'div' ) )
				->attr( 'class', array( 'd-grid', 'gap-4', 'grid-two-equal' ) )
				->child(
					( new HTML_Refactory( 'div' ) )
						->child(
